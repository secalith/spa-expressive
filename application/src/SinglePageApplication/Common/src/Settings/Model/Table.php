...
class Table
{
    ...
    public function fetchAllBy($value,$name="name")
    {
    ...
    {
        $rowset = $this->tableGateway->select(array('name' => $uid));
        $row = $rowset->current();
        if (!$row) {
    ...
    public function fetchBy($value,$name="name")
    {
    ...
    {
        $this->tableGateway->delete(array('name' => $uid));
    }
}